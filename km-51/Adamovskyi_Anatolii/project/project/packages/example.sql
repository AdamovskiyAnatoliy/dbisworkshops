create or replace package user_auth is
  ...
  procedure new_user(user_id OUT integer,
                     status out varchar2,
...
create or replace package body user_auth is
  ...
  begin
    ...
    from "user"
    where trim("user".user_email) = trim(email)
      and trim("user".user_password) = trim(pass);
    return counter;
  end is_user;
  ...
  begin
    ...
    BEGIN
      ...
      values (SEQ_USER.nextval, USER_STUDYBOOK, USER_YEAR, USER_NAME, USER_EMAIL, USER_BIRTHDAY,
              USER_PASSWORD) RETURNING USER_ID
               INTO user_id;
